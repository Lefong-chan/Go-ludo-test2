...
onMounted(async () => {
  ...
  // ── Username: tsy mamaky avy amin'ny localStorage intsony
  //    Ny /api/session?action=check-session no loharano marina tokana
  if (savedFirebaseUid) {
...
const fetchAndCheckUsername = async (fbUid) => {
  try {
    ...
    if (res.ok) {
      ...
      // Token lany → tsy miditra intsony
      showUsername.value = true
    } else {
      showUsername.value = true
    }
