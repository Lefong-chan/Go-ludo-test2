    ...
    <div class="mdl">
      ...
      <div class="profile-header">
        ...
        <div class="identity-block">
          ...
          <div class="p-uid-row">
            <div class="p-uid">
              ...
              <button
                ...
                :disabled="uidCopied"
                @click="copyUid"
                :title="uidCopied ? 'Copié !' : 'Copier l\'UID'"
              >
          ...
          <div v-if="isViewer" class="viewer-presence" :class="viewerOnline ? 'vpres-on' : 'vpres-off'">
            <span v-if="viewerOnline">● En ligne</span>
      ...
      <template v-if="isViewer">
        ...
        <div class="viewer-actions">
          ...
          <!-- Chat -->
          <button class="vbtn vbtn-chat" @click="emitMessage">
    ...
    <Transition name="sub-fade">
      <div v-if="showAvatarPicker" class="sub-ovl" @click.self="showAvatarPicker = false">
        <div class="sub-box">
          ...
          <h3 class="sub-title">Choisissez votre avatar</h3>
          <p class="sub-hint">Appuyez pour choisir</p>
    ...
    <Transition name="sub-fade">
      <div v-if="showEmailModal" class="sub-ovl" @click.self="showEmailModal = false">
        <div class="sub-box sub-box-sm">
          ...
          <h3 class="sub-title">Email du compte</h3>
          <div class="email-display">
            <span class="material-icons" style="font-size:16px; color:#ffecaa;">verified_user</span>
            <span class="email-text">{{ userEmail }}</span>
          </div>
          <p class="email-note">Il s'agit de l'email associé à votre compte Go Ludo.</p>
          <button class="btn-close-email" @click="showEmailModal = false">Fermer</button>
        </div>
      </div>
    </Transition>
  </Teleport>
...
const emit = defineEmits([
  ...
  // Viewer events
  'accept-request', 'decline-request', 'send-request', 'remove-friend', 'challenge', 'message',
])
...
const emitMessage = () => emit('message', props.viewerData.firebaseUid)
...
const selectAvatar = async (emoji) => {
  ...
  try {
    const res  = await fetchWithTimeout('/api/session', {
      ...
      body:    JSON.stringify({ action: 'setAvatar', uid: props.userFirebaseUid, avatar: emoji }),
    })
...
const fetchAndShowEmail = async () => {
  ...
  try {
    const res = await fetchWithTimeout('/api/session', {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({ action: 'getEmail', uid: props.userFirebaseUid }),
    })
    const data = await res.json()
    userEmail.value = (res.ok && data.email) ? data.email : '—'
  } catch (e) {
    userEmail.value = '—'
    setTimeout(() => { errorMsg.value = e?.message || 'Erreur réseau. Réessayez.' }, 30)
  }
...
.mdl {
  ...
  border: 4px solid rgba(255, 220, 100, 0.15);
...
/* "gap" ao anaty flexbox — tsy takatry ny navigateur/WebView taloha
   ...
   0px, ka lasa mifanakaiky ny avatar sy ny anarana/UID. Solon'ny gap
   dia "margin-left" eo amin'ny zanaka rehetra afa-tsy ny voalohany. */
.profile-header {
  display: flex; align-items: center;
  margin-bottom: 20px;
}
.profile-header > * + * { margin-left: 18px; }
...
.uid-val { color: rgba(255,245,200,.65); margin-left: 5px; }
...
.btn-copy-uid {
  ...
  flex-shrink: 0; margin-left: 7px; /* teo aloha: gap:5px (.p-uid) + margin-left:2px (io ihany) */
}
...
.balance-icon   { font-size: 26px; color: #ffd966; flex-shrink: 0; margin-right: 12px; }
...
.balance-unit   { font-size: 11px; font-weight: 700; color: rgba(255,217,102,.4); text-transform: uppercase; letter-spacing: 1px; align-self: flex-end; padding-bottom: 4px; margin-left: 12px; }

/* ══ ACTION BUTTONS (mode propre) ══════════════════════════════ */
/* "gap" ao anaty flexbox (column sy row) — tsy takatry ny navigateur
   taloha (jereo ny fanazavana etsy ambony), solon'ny "margin". */
...
/* "margin-right" amin'ny icone (fa tsy "> * + *") satria soratra
   "bare" (tsy voafono anaty span) no manaraka azy anaty .vbtn, ka tsy
   voakasik'ny "child combinator" selector (izay "element" ihany no
   voakasiny, tsy "text node"). */
.vbtn .material-icons, .vbtn .vbtn-spin { margin-right: 10px; font-size: 19px; flex-shrink: 0; }
...
.vbtn-chat {
  background: linear-gradient(135deg, rgba(57,185,255,.18), rgba(15,100,200,.1));
  border: 1px solid rgba(57,185,255,.32); color: #a8d8ff;
}
.vbtn-chat .material-icons { color: #39b9ff; }
...
.email-text { font-size: 14px; font-weight: 700; color: #c8e8ff; word-break: break-all; margin-left: 8px; }
