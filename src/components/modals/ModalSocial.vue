          ...
          <div v-for="f in filteredFriends" :key="f.firebaseUid" class="fi">
            <div class="fi-l">
              <div class="fava">
                ...
                <span :class="['pres-dot', getPresence(f.firebaseUid).online ? 'pres-on' : 'pres-off']"></span>
              </div>
              <div>
...
// presenceMap: { [firebaseUid]: { online: bool, lastSeen: number|null } }
...
// Mametraka listener RTDB ho an'ny uid iray
...
// Manafoana listener RTDB ho an'ny uid iray
...
// Manafoana listeners RTDB rehetra
...
// ── formatLastSeen ─────────────────────────────────────────────
// Mampiseho:
//   - "Offline" raha tsy misy lastSeen
//   - "X min ago"  (< 1h)
//   - "X hr ago"   (< 1d)
//   - "X days ago" (≥ 1d)
...
// Buddies sorted: Online ambony, Offline ambany
...
const startFriendsListener = () => {
  ...
  unsubFriends = fsOnSnapshot(colRef, snap => {
    ...
    // Sync presence listeners: asio vaovao, esorina tsy ilaina
    const acceptedUids = new Set(
      newList.filter(f => f.status === 'accepted').map(f => f.firebaseUid)
    )
    // Asio listener ho an'ny accepted vaovao
    ...
    Object.keys(presenceUnsubs).forEach(uid => {
      if (!acceptedUids.has(uid)) unsubscribePresence(uid)
    })
...
// ── sendRequest: fanamarinana client side ──────────────────────
...
// ── acceptRequest: fanamarinana client side ────────────────────
...
.pres-on  { background:#3ddc84; box-shadow:0 0 5px rgba(61,220,132,.7); }
.pres-off { background:#666; }
