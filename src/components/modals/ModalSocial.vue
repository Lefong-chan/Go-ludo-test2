<template>
  ...
  >
    <div class="mdl" @click="closePopup">
        ...
        <div class="flist" ref="listSearch">
          ...
          >
            ...
            <div class="fa">
              ...
              <button v-else-if="isPendingSent(p.firebaseUid)" class="fb fb-c" disabled style="min-width:66px;">Pending</button>
      ...
      <Transition name="pop-anim">
        ...
        >
          ...
          <template v-else-if="popup.tab === 'search'">
            ...
            <!-- Add button — aseho raha tsy friend/pending -->
            <button
...
// ── Confirm: famafa amis ───────────────────────────────────────
const confirmRemove  = reactive({ visible: false, user: null, username: '' })
const confirmDecline = reactive({ visible: false, user: null, username: '' })
...
const doRemoveFriend = async () => {
  ...
  await apiCall('remove-' + confirmRemove.user.firebaseUid, 'remove-friend', { friendFirebaseUid: confirmRemove.user.firebaseUid })
  confirmRemove.user = null
}
const doDeclineRequest = async () => {
  if (!confirmDecline.user) return
  await apiCall('decline-' + confirmDecline.user.firebaseUid, 'decline-request', { requesterFirebaseUid: confirmDecline.user.firebaseUid })
  confirmDecline.user = null
}

// ── Popup cadre kely ───────────────────────────────────────────
// popup.offsetTop: distance avy amin'ny top an'ilay .flist
// Apetraka eo ambanin'ilay row notsindriana
const popup = reactive({
  visible: false,
  uid:     '',
  tab:     '',
  user:    null,
  top:     0,   // px avy amin'ny top an'ilay .flist (scrolled)
})

// Refs ho an'ny .flist container tsirairay
...
// popupStyle: mifanandrify amin'ny horizontal amin'ilay modal,
// eo ambanin'ilay row notsindriana (vertical)
const POPUP_WIDTH = 200 // px
...
const togglePopup = async (user, tab, event) => {
  // Ra ilay iray efa misokatra dia atohy
  if (popup.visible && popup.uid === user.firebaseUid && popup.tab === tab) {
    closePopup(); return
  }

  // Manampy subscription presence ho an'ny search results
  ...
  // Kajy ny position vertical: eo ambanin'ilay row notsindriana
  await nextTick()
  const listEl = getListRef()
  if (listEl) {
    // getBoundingClientRect an'ilay row sy an'ilay list container
    const rowEl     = event.currentTarget
    const listRect  = listEl.getBoundingClientRect()
    const rowRect   = rowEl.getBoundingClientRect()
    // Top = eo ambanin'ilay row, soroina ny scroll position an'ilay list
    popup.top = (rowRect.bottom - listRect.top) + listEl.scrollTop + 4
  }
...
const getApiBase = (action) =>
  ['send-request','accept-request','decline-request'].includes(action) ? API_REQUESTS : API_FRIENDS
...
.popup-card {
  ...
  min-width: 180px;
  /* Tsy azo tsindrina ny row eo ambaniny */
  pointer-events: auto;
}
...
.pop-anim-enter-from, .pop-anim-leave-to { opacity: 0; transform: translateX(-50%) scaleY(.85); }
</style>
