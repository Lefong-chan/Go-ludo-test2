...
// popupStyle: eo ambanin'ilay button ankavanana, aligned right
...
const togglePopup = async (user, tab, event) => {
  ...
  // Kajy ny position vertical: eo ambanin'ilay button ankavanana (.fa)
  await nextTick()
  ...
  if (listEl) {
    ...
    // Mitady ilay .fa button eo ankavanana
    const faEl     = rowEl.querySelector('.fa')
    const refRect  = faEl ? faEl.getBoundingClientRect() : rowRect
    // Top = eo ambanin'ilay button + elanelana kely 6px
    popup.top = (refRect.bottom - listRect.top) + listEl.scrollTop + 6
  }
...
.flist {
  ...
  gap:7px; overflow-y:auto; overflow-x:visible; flex:1; padding-right:4px;
  position:relative;
}
